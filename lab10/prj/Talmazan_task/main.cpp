...
int checkXYZ(double num){
    ...
    if(!(wcin >> num)){
        wcin.clear();
        while (wcin.get() != '\n');
        wcout << L"\nВи ввели символ, введiть число." << endl;
        checkXYZ(num);
    }
    return num;
}
...
int main()
{
    ...
    wcout << L"| * search_word - шукає чи є в вхiдному файлi слова з задачi та вносить до файлу| " << endl;
    wcout << L"|   iнформацiю про розробника та кiлькiсть символiв у введеному реченнi.        | " << endl;
    wcout << L"| * second_funct - перелiк Державних символiв та дата дозапису iнформацiї.      | " << endl;
    ...
    wcout << L"\nСтворiть файл input.txt та введiть його назву з розширенням .txt(Ви створюєте вхiдний файл input.txt самостiйно через файловий менеджер у тецi з програмою): ";
    ...
    while(true){
        if(access(ch_inputName, 0) != -1){
            ...
            wcout << L"\nФайл не знайдено. Можливо невiрно введенне iм'я або вiдсутнiсть файлу у тецi програми." << endl;
            wcin.getline(inputName, 50);
            wcstombs(ch_inputName, inputName, sizeof(ch_inputName));
        }
    }
    ...
    if(wcsstr(F, Key)){
        ...
        while(true){
            ...
            case 'W':
                ...
                wcout << L"\nВведiть назву вхiдного файлу input.txt: ";
                task10_1();
                first_funct();
